...
namespace glfw {
class Keyboard {
  friend void WndKeyCallback(GLFWwindow *window, int key, int scancode,
                             int action, int mod);
...
private:
  struct Event {
    ...
    Stat stat;
    uint32_t key_code;
    Event(Stat stat = Stat::INVALID, uint32_t key_code = 0) noexcept
...
public:
  ...
  inline void ClearState() noexcept { key_stat_.reset(); };
