...
namespace util {
Config::Section::Section(std::string &&name) noexcept {
    Init(std::move(name));
}
...
bool Config::Section::IsInit() const noexcept {
    return !name_.empty();
}

void Config::Section::WriteConf(const std::string &key, const std::string &value) {
...
const std::string &Config::Section::ReadConf(const std::string &key) const {
    // 查找是否该key是否已经存在
    auto it = std::find_if(config_.cbegin(), config_.cend(),
                           [&key](const Info &cur) -> bool { return cur.key == key; });
    if (it == config_.cend()) {
...
Config::Section &Config::GetSection(std::string &&section_name) {
    auto it = std::find_if(
        ...
        [&section_name](const Section &cur) -> bool { return cur.name_ == section_name; });
    if (it == sections_.end()) {
